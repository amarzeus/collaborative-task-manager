...
exports.analyticsService = {
     ...
     */
    async getCompletionTrends(userId, days = 7) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        startDate.setHours(0, 0, 0, 0);
        // Get completed tasks per day
        const completions = await prisma_js_1.prisma.taskHistory.groupBy({
            by: ['createdAt'],
            where: {
        ...
        const creations = await prisma_js_1.prisma.taskHistory.groupBy({
            by: ['createdAt'],
            where: {
                action: 'created',
                createdAt: {
        ...
        for (let i = days - 1; i >= 0; i--) {
            ...
            const dayCompleted = completions.filter((c) => {
                const cDate = new Date(c.createdAt);
                cDate.setHours(0, 0, 0, 0);
                return cDate.getTime() === date.getTime();
            }).reduce((sum, c) => sum + c._count, 0);
            const dayCreated = creations.filter((c) => {
                const cDate = new Date(c.createdAt);
                cDate.setHours(0, 0, 0, 0);
                return cDate.getTime() === date.getTime();
            }).reduce((sum, c) => sum + c._count, 0);
    ...
    async getProductivityMetrics(userId) {
        ...
        const completedThisWeek = await prisma_js_1.prisma.taskHistory.count({
            where: {
                action: 'status_changed',
                newValue: 'COMPLETED',
                createdAt: {
        ...
        const completedTasks = await prisma_js_1.prisma.taskHistory.findMany({
            where: {
                action: 'status_changed',
                newValue: 'COMPLETED',
                task: {
        ...
        const avgCompletionDays = completedTasks.length > 0
            ? totalTime / completedTasks.length / (24 * 60 * 60 * 1000)
            : 0;
        return {
            completedThisWeek,
    ...
    async getInsights(userId) {
        ...
        const overdueTasks = assignedTasks.filter((t) => new Date(t.dueDate) < new Date() && t.status !== 'COMPLETED');
        const highPriorityPending = assignedTasks.filter((t) => (t.priority === 'HIGH' || t.priority === 'URGENT') && t.status !== 'COMPLETED');
        const inProgressTasks = assignedTasks.filter((t) => t.status === 'IN_PROGRESS');
        // Generate insights
        if (overdueTasks.length > 0) {
            insights.push(`You have ${overdueTasks.length} overdue task${overdueTasks.length > 1 ? 's' : ''}. Consider prioritizing them.`);
        }
        if (highPriorityPending.length > 3) {
            insights.push(`You have ${highPriorityPending.length} high-priority tasks pending.`);
        }
        if (inProgressTasks.length > 5) {
            insights.push(`You have ${inProgressTasks.length} tasks in progress. Focus on completing some before starting new ones.`);
        }
        ...
        if (productivity.completedThisWeek > 10) {
            insights.push(`Impressive! You've completed ${productivity.completedThisWeek} tasks this week.`);
        }
