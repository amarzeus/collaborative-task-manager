...
export const analyticsService = {
  ...
  async getEfficiencyMetrics(userId: string, scope: AnalyticsScope = 'personal') {
    ...
    tasks.forEach((task) => {
      ...
      for (let i = 0; i < h.length - 1; i++) {
        ...
        const next = h[i + 1];
        const duration =
          (next.createdAt.getTime() - current.createdAt.getTime()) / (24 * 60 * 60 * 1000);
    ...
    return [
      {
        status: 'TODO',
        avgDays: Math.round((times['TODO'].total / (times['TODO'].count || 1)) * 10) / 10,
      },
      {
        status: 'IN_PROGRESS',
        avgDays:
          Math.round((times['IN_PROGRESS'].total / (times['IN_PROGRESS'].count || 1)) * 10) / 10,
      },
      {
        status: 'REVIEW',
        avgDays:
          Math.round((times['REVIEW'].total / (times['REVIEW'].count || 1)) * 10) / 10 || 0.5,
      },
    ];
  },
  ...
  async getInsights(userId: string, scope: AnalyticsScope = 'personal', days: number = 7) {
    ...
    if (overdueTasks.length > 0) {
      insights.push(
        `${scope === 'personal' ? 'You have' : 'There are'} ${overdueTasks.length} overdue tasks that need attention.`
      );
    }

    if (productivity.completedThisWeek > 10) {
      insights.push(
        `Exceptional week! Your throughput is ${Math.round((productivity.completedThisWeek / 5) * 100)}% above your baseline.`
      );
    } else if (productivity.completedThisWeek > 0) {
      insights.push(
        `Maintain momentum. ${productivity.completedThisWeek} tasks completed this period.`
      );
    }

    if (productivity.performanceScore > 800) {
      insights.push(
        "Consistency reached 'Elite' status. Your lead time is among the top 10% of users."
      );
    }
    ...
    if (urgentTasks.length > 0) {
      insights.push(
        `Urgent focus required: ${urgentTasks.length} high-impact tasks are still open.`
      );
    }
