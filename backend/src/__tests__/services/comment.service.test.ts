...
describe('CommentService', () => {
    ...
    describe('getCommentsByTaskId', () => {
        ...
        it('should throw 404 when task does not exist', async () => {
            ...
            await expect(
                commentService.getCommentsByTaskId('non-existent')
        ...
        it('should throw 404 when task does not exist', async () => {
            ...
            await expect(
                commentService.createComment(
                    { content: 'Test', taskId: 'non-existent' },
                    'user-1'
                )
            ).rejects.toThrow(AppError);
        });
    ...
    describe('deleteComment', () => {
        ...
        it('should throw 403 when non-author tries to delete', async () => {
            ...
            await expect(
                commentService.deleteComment('comment-1', 'hacker')
            ).rejects.toThrow(AppError);
        });
        ...
        it('should throw 404 when comment does not exist', async () => {
            ...
            await expect(
                commentService.deleteComment('non-existent', 'user-1')
            ).rejects.toThrow(AppError);
        });
    });
});
