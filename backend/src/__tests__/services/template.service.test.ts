...
describe('TemplateService', () => {
    ...
    describe('getTemplateById', () => {
        ...
        it('should throw 403 when accessing private template not owned by user', async () => {
            ...
            await expect(
                templateService.getTemplateById('template-1', 'other-user')
        ...
        it('should throw 404 when template does not exist', async () => {
            ...
            await expect(
                templateService.getTemplateById('non-existent', 'user-1')
            ).rejects.toThrow(AppError);
        });
    });
    ...
    describe('deleteTemplate', () => {
        ...
        it('should throw 403 when non-creator tries to delete', async () => {
            ...
            await expect(
                templateService.deleteTemplate('template-1', 'other-user')
            ).rejects.toThrow(AppError);
        });
        ...
        it('should throw 404 when template does not exist', async () => {
            ...
            await expect(
                templateService.deleteTemplate('non-existent', 'user-1')
            ).rejects.toThrow(AppError);
        });
    });
