...
import { aiService, AI_FUNCTIONS, ChatMessage, AIContext } from '../services/ai.service.js';
  ...
  if (lowerMessage.includes('create') && lowerMessage.includes('task')) {
    ...
    const result = await aiService.executeFunction(
      'create_task',
      {
        ...
        priority: lowerMessage.includes('urgent')
          ? 'URGENT'
  ...
  if (
    ...
    if (result.success) {
      ...
      const taskList = tasks
        .slice(0, 5)
        .map((t: any, i: number) => `${i + 1}. **${t.title}** [${t.priority}] - ${t.status}`)
        .join('\n');
